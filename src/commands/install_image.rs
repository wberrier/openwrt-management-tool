...
pub fn install_image(config: &Config) -> Result<()> {
    println!("Installing image for: {}", &config.name);
    ...
    if image_path.exists() {
        ...
        let _code = getstatus_shell(format!("ssh root@{} \"[[ \\$(sha256sum /tmp/image.bin | awk '{{print \\$1}}') = \"{}\" ]] && sysupgrade -v -n /tmp/image.bin\"", &config.name, &local_hash))?;
    } else {
